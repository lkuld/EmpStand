...
*Dieses Tutorial ist ein Schnellkurs durch die Grundlagen statistischer Tests. Wenn Sie nicht allem folgen können, ist das okay. Die nächsten Wochen werden wieder deutlich angewandter. Sinn dieses Tutorials (neben neuen R-Funktionalitäten) ist es, eine Ahnung zu bekommen worauf statistische Tests beruhen.*
...
sample(x1,3) # hier werden die Bestandteile der Funktion per Stelle erkannt. Wir könnten auch sample(x=x1,size=3) oder sample(size=3, x=x1) schreiben, nicht aber sample(3,x1) 
```

R hat verschiedene Zufallsalgorithmen implementiert. Wie bei allen Funktionen, können Details per Fragezeichen abgerufen werden, also `?sample`. Falls wir für eine Simulation immer dasselbe Ergebnis haben wollen, können wir den Zufallsalgorithmus per `set.seed(seed)` initiieren. Klicke mehrfach auf Run Code und verändere die Parameter (Inputs).
...
Unser Würfel war ein Beispiel für eine diskrete Verteilung (diskrete oder abgetrennte Ereignisse). Daneben gibt es auch kontinuierliche Verteilungen, wie z.B. die Gleichverteilung zwischen $1$ und $3$ unten. 

Wenn wir eine Zahl ziehen, kommt diese aus jedem Abschnitt zwischen $1$ und $3$ gleicher Größe mit gleicher Wahrscheinlichkeit. Sie liegt also z.B. mit Wahrscheinlichkeit $0,25$ zwischen $1,1$ und $1,6$ ($P(1,1<X<1,6)=0,25$) und mit dieser Wahrscheinlichkeit auch aus zwischen $1,3$ und $1,8$ (jeweils der Flächeninhalt). Die Funktion unten zeichnet eine Illustration der Verteilung und gibt die Wahrscheinlichkeit, dass eine Zahl zwischen einem beliebigen Minimum und Maximum liegt. Verändere die Parameter, um zu sehen wie sich die Wahrscheinlichkeit verändert
...
* Der zentrale Grenzwertsatz begründet warum wir bei Summen unabhängiger Zufallsgrößen annähernd eine Normalverteilung beobachten. Die Mittelwerte in der letzten Aufgabe sind solche Summen. Zwar zusätzlich durch die Anzahl geteilt, aber das beeinflusst nur Erwartungswert und Varianz ($\frac{1}{n} \sum_{i=1}^n x_i$). Eine Sonderform der Normalverteilung ist in der nächsten Graphik abgebildet: die sogenannte Standardnormalverteilung mit Erwartungswert null und Varianz eins. Wenn eine Zufallsvariable Z einer Normalverteilung mit Erwartungswert null und Varianz eins folgt notiert man das als $Z \sim N(0,1)$.*
...
Wir benutze eine Schleife mit der Syntax `for(i in ...) { tu was mit i}`. `...` ist ein beliebiger Vektor, z.B. 1 bis 10 (`1:10`). `i` ist eine Variable (kann auch anders genannt werden). Innerhalb der geschweiften Klammer führen wir dann etwas aus, z.B. `3+i`.

Eine Liste in R ist eine Sammlung beliebiger Objekte, auf die dann per eckiger Klammer zugegriffen werden kann. Z.B. ist Liste[[10]] das zehnte Element einer Liste. Also MittelwertListe[[i]] das i-te Element.
...
Wir wollen nun sehen, ob der Abstand zum Erwartungswert tatsächlich ungefähr einer Normalverteilung folgt. Zunächst ziehen wir 170 ab. Dann zeigen wir den Abstand als Histogramm. Wir sehen, das die Verteilung ungefähr um null liegt, aber etwas un-rund und auch mit etwas größerer Verteilung.
...
Als nächstes teilen wir durch die Stichprobenvarianz $\sqrt{\frac{\sigma^2}{n}}$. Das ist die Wurzel der Varianz geteilt durch die Stichprobengröße , also $\sqrt{\lambda/n}$ oder $\sqrt{170/100)$. Das ergibt nun asymptotisch eine Teststatistik mit Standardnormalverteilung (asymptotisch $\sim$ bei unendlich vielen Wiederholungen), die z-Wert genannt wird. Eine Standardnormalverteilung ist eine Normalverteilung mit Erwartungswert null und Varianz eins. Das hat den Vorteil das wir unsere ursprüngliche Stichprobe mit einer Verteilung in Bezug gesetzt haben, die wir sehr gut kennen. Dadurch können wir beurteilen, wie wahrscheinlich es ist einen bestimmten Wert in der Stichprobe zu beobachten, falls alle Annahmen gegeben sind (hauptsächlich Poisson mit $\lambda=170$ und Beobachtungen unabhängig.) Für weitergehende Infos zu Zweck und Konstruktion, siehe auch die Wikipediaseite [Standardisierung](https://de.wikipedia.org/wiki/Standardisierung_(Statistik)).
