...
Was auffaellt sind die vielen `NA` Beobachtungen. Das steht fuer _Not Available_, also fehlendem Wert in dieser Zelle. Ursache kann auch ein unsinniger Datentyp sein,  z.B. eine numerisches Wort:`as.numeric("rot")` vs. `as.numeric("3")`.


Wie groß ist unser Datensatz? Probiere es mit `length(Bairoch_1988)`. Das Ergebnis zeigt, dass der Datensatz eine Kombination von 14 Variablen ist. Alle haben die gleiche Länge, aber können unterschiedliche Typen (Text, Zahlen, ...) haben. Das Format ist das Standardformat in R: ein `data.frame`. Ein `data.frame` ist also eine Liste (`list`) bei der alle Elemente Vektoren der selben Laenge sind.

Die Anzahl der Reihen bekommen wir per `nrow(Bairoch_1988)`. Die Namen der Variablen per `names(Bairoch_1988)`. `summary(Bairoch_1988)` gibt einen Überblick für alle Variablen.
...
Oder fuer die Staedte in Frankreich:

```{r wahr2, exercise=TRUE}


Bairoch_1988$country == "France"
...
Wir werden jetzt Variablen miteinander verrechnen. Zunächst möchten wir die Stadtgrößen in 1850 in hunderttausend statt tausend Einwohnern zählen. Die neue Variable soll `Jahr_1850_hunderttausend` heißen.
...
Umgekehrt, könnten wir auch die Jahreszahl extrahieren. Das ist es etwas komplizierter, da diese Zahlen unterschiedlich sind, d.h. wir müssen verschiedene Werte zulassen. Dazu benutzten wir, wie in anderen Programmiersprachen auch, eine _regular expression_. Diese _regular expressions_ sind eine standardisierte Form auf Zeichen zuzugreifen. `"e.n$""` wuerde z.B. auf alle Kombinationen zutreffen, die (1) am Ende eines _strings_ stehen, da `$` das Ende des Strings bedeutet. (2) Aus e, irgendetwas und n bestehen, da der Punkt `.` alle Zeichen ‚matcht‘. Wir würden also z.B. in "Farbein", die letzten drei Buchstaben 'matchen'.  Ein guter Überblick über _regular expressions_ bietet die Website https://www.regular-expressions.info/.

Für unser Beispiel können wir die Funktion `str_extract` mit der _regular expression_ `"[0-9]+"` nutzen. Eckige Klammern lassen alles zu was in ihnen steht als einzelnes Element. Z.B. matcht "[BS]aum" die Wörter Baum und Saum. Der Bindestrich greift auf die Ordnung der Zeichen zurück, hier z.B. [0-9] ist das gleiche wie [0123456789]. [m-o] wäre [mno]. Eine geschweifte Klammer mit Zahl (z.B. `{4}`) bedeutet, dass das vorangehende Element so oft erscheinen muss. [0-9]{4} würde also vier Zahlen matchen. In unserem Beispiel haben wir aber Jahreszahlen mit drei Stellen. Deswegen suchen wir nur Zahlen ([0-9]), die mindestens einmal, aber beliebig oft vorkommen per `+`.

Versuche im naechsten Kasten das obige Ergebnis mit `str_extract` zu erzielen.
...
Eine große Stärke von R sind Graphiken. Viele Medien (z.B. BBC) benutzten Graphiken direkt aus R, die mit relative wenig Aufwand produziert werden können. Diese Graphiken nutzten das Paket `ggplot2`. Das schauen wir uns in einem späteren Tutorial an. Falls du Graphiken z.B. für eine Seminararbeit anfertigst, schau dir das Paket aber auf jeden Fall bereits an. Es gibt dazu sehr viele online Ressourcen.
